...
class BlackJack:        #클래스 BlackJAck
    def pressedB50(self):
        self.betMoney += 50
        if 50 <= self.playerMoney:  #베팅 한도 검사
    ...
    def pressedB10(self):
        self.betMoney += 10
        if 10 <= self.playerMoney:  # 베팅 한도 검사
            self.LbetMoney['text'] = '$' + str(self.betMoney)
            self.playerMoney -= 10
            self.LplayerMoney['text'] = 'You have $' + str(self.playerMoney)
            self.Deal['state'] = 'active'
    ...
    def pressedB1(self):
        self.betMoney += 1
        if 1 <= self.playerMoney:  # 베팅 한도 검사
            self.LbetMoney['text'] = '$' + str(self.betMoney)
            self.playerMoney -= 1
            self.LplayerMoney['text'] = 'You have $' + str(self.playerMoney)
            self.Deal['state'] = 'active'
    ...
    def checkWinner(self):
        ...
        self.LcardsDealer[0].image = p      #라벨 이미지 레퍼런스를 갖고 있어야 이미지가 보인다
    ...
    def hitPlayer(self,n):  #n: 카드 이미지 라벨 위치 변수
        ...
        p = PhotoImage(file='BlackJack Resource/cards/'+newCard.filename()) #Card 객체의 filename 으로 이미지 생성
        self.LcardsPlayer.append(Label(self.window,image=p))    #플레이어 카드 이미지 라벨 리스트에 추가
        self.LcardsPlayer[self.player.inHand()-1].image = p     #파이썬에서 라벨 이미지 레퍼런스를 지정해야 이미지가 보인다
        self.LcardsPlayer[self.player.inHand() - 1].place(x=250+n*30,y=350) #n: 위치 변수에 따라서 x 위치가 달라진다
    ...
    def hitDealerDown(self):    #딜러 첫번째 카드 뒤집어서 생성
        ...
        p = PhotoImage(file='BlackJack Resource/cards/b2fv.png')  # 뒤집어진 카드 이미지 생성
        self.LcardsDealer.append(Label(self.window, image=p))  # 플레이어 카드 이미지 라벨 리스트에 추가
        self.LcardsDealer[self.dealer.inHand() - 1].image = p  # 파이썬에서 라벨 이미지 레퍼런스를 지정해야 이미지가 보인다
        self.LcardsDealer[self.dealer.inHand() - 1].place(x=250, y=150)  # 고정된 위치
    ...
    def deal(self):
        ...
        #카드 덱 52장 셔플 0,1,...,51
        self.cardDeck = [i for i in range(52)]  #리스트 내장 기법
        random.shuffle(self.cardDeck)   #셔플링
        self.deckN = 0      #카드덱 인덱스 = 0 초기화

        self.hitPlayer(0)   #카드 이미지 라벨 위치 0에 카드 생성
        self.hitPlayer(1)   #카드 이미지 라벨 위치 1에 카드 생성
        self.hitDealerDown()    #딜러 카드 첫번째 이미지 라벨은 뒤집어서 생성(위치 고정)
    ...
    def setupLabel(self):
        self.LbetMoney = Label(text = '$0', width=4, height=1, font=self.fontstyle,bg='green',fg='cyan')    #베팅머니 라벨
        self.LbetMoney.place(x=200,y=450)
        self.LplayerMoney = Label(text='You have $1000', width=15, height=1, font=self.fontstyle, bg='green', fg='cyan')    #플레이어 머니 라벨
        ...
        self.Lstatus = Label(text='', width=15, height=1, font=self.fontstyle, bg='green', fg='white')  #베팅머니 라벨
    ...
    def __init__(self):
        ...
        self.setupButton()      #버튼 설정 함수 호출
        self.setupLabel()       #라벨 설정 함수 호출
        ...
        self.nCardsDealer = 0           #딜러 카드 이미지 라벨의 위치 변수
        self.nCardsPlayer = 0           #플레이어 카드 이미지 라벨의 위치 변수
        self.LcardsPlayer = []          #플레이어 카드 이미지 라벨 리스트
        self.LcardsDealer = []          #딜러 카드 이미지 라벨 리스트
        self.deckN = 0                  #카드 덱 (0..51) 숫자를 셔플해서 갖고 있다. 카드 덱의 인덱스 변수
