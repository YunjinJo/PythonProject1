...
class TetrisApp:
        ...
        for y, row in enumerate(self.stone):    #(y,row) = (0,[0,2,2]),(1,[2,2,0])
            for x, val in enumerate(row):       #(x,val) = (0,0), (1,2), (2,2)
                if 0<=y + off_y<self.rows and 0<=x+off_x<self.cols: #유효한 인덱스 검사
                    if val and self.gridCell[y+off_y][x+off_x]: #stone도 비어 있지 않고 보드도 비어 있지 않으면
                        return True
                else:   #유효한 인덱스가 아니라면
                    return True
        return False
    ...
    def new_stone(self):
        ...
        self.stone = tetris_shapes[random.randrange(len(tetris_shapes))]    #[0,1,2,...,6] 중에서 랜덤하게 선택
        #stone x 위치 = 게임판 중간(self.cols/2) -  stone 의 너비의 절반(self.stone[0]/2)
        self.stone_x = self.cols//2 - len(self.stone[0]) // 2
        #stone y 위치
        self.stone_y = 0

        if self.check_collision((self.stone_x,self.stone_y)):   #self.stone이 self.board의 다른 블록과 충돌 검사
            self.gameover = True
    ...
    def paintGrid(self):
        ...
        #self.stone 라벨 색 칠하기
        for y, row in enumerate(self.stone):  # 인덱스와 값을 추출, (y,row)=(0,[0,2,2]), (1,[2,2,0]) 0-> 인덱스 [0,2,2] -> 값
    ...
    def rotate(self):
        # 예 self.stone = [[1, 1, 1],
        #                  [0, 1, 0]]
        # y좌표는 0, 1, x좌표는 2, 1, 0 즉 0열의 2개, 1열의 2개, 2열의 2개 묶음
        #rotate_left:     [[1, 0],
        #                  [1, 1],
        #                  [1, 0]]
        # rotate_l = [[a[y][x] for y in range(len(a))] for x in range(len(a[0])-1,-1,-1]
        # rotate_l = [[self.stone[y][x] for y in range(len(self.stone))] for x in range(len(self.stone[0])-1,-1,-1)]

        #y좌표는 1, 0, x좌표는 0, 1, 2 즉 0열의 2개, 1열의 2개, 2열의 2개 묶음
        #rotate_right:    [[0, 1],
        #                  [1, 1],
        #                  [0, 1]]
        #b = [[a[y][x] for y in range (len(a))] for x in range(len(a[0]))] -> 전치행렬

        #rotate_r = [[a[y][x] for y in range(len(a)-1,-1,-1)] for x in range(len(a[0])]
        #rotate_r = [[self.stone[y][x] for y in range(len(self.stone)-1,-1,-1)] for x in range(len(self.stone[0]))]
    ...
    def check_remove(self):
        # 맨 밑줄에서 부터 rows-1,...0 으로 한줄이 채워졌으면 없애고 self.gridCell 앞에 [0,0,0,0,...0]을 붙임
        i = self.rows-1 #맨 밑에줄 인덱스 부터
        while i >= 0:   #i = rows-1,...,0
            if not 0 in self.gridCell[i]:    #i 행에 0이 없으면 self.gridCell = [[1,2,1,2,1,2],[0,1,0,1,2,1,1],...] 0 이 하나라도 있으면 채워진게 아니다
                del (self.gridCell[i])       #i행 삭제
                self.gridCell = [[0 for _ in range(self.cols)]] + self.gridCell
            else:   #i번 행이 채워지지 않았으면 i를 1 감소소
                i -= 1
    ...
    def move_down(self):
        if self.check_collision((self.stone_x,self.stone_y+1)): #stone을 한 칸 아래로 움직일 때 충돌 검사
            ...
            self.new_stone()    #새로운 stone 생성
            self.check_remove() #한줄 채워진 줄은 지운다

        else:   #충돌이 안 일어났으면
            self.stone_y += 1   #stone은 1칸 아래로 이동한다
        self.paintGrid()
