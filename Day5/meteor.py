...
METEO_MAX = 30      #유성의 개수
mx = [0]*METEO_MAX  #mx = [0,0,...,0] 0이 30개
my = [0]*METEO_MAX

key = ''        #키 값을 대입할 전역변수
koff = False    #키를 눌렀다 뗐을 때 사용하는 bool 변후, MAC을 지원하기 위한 용도
def key_down(e):    #키를 눌렀을 때 실행되는 함수
    global key  #전역변수 참조를 알림
    key = e.keysym
    koff = False

def key_up(e):  #키를 뗐을 때 실행되는 함수
    global koff
    koff = True
...
def main():
    ...
    bg_pos = (bg_pos + 1)%640   #bg_pos = 0,1,2,...,639,0,1,...
    ...
    if index == 0:  #타이틀 화면이면
        ...
        if key == 'space':  #게임 시작을 알림
            score = 0   #점수 초기화
            px = 240    #우주선의 위치를 x좌표 한 가운데로 초기화
            init_enemy()    #유성들을 초기화
...
def init_enemy():   #유성을 초기화
    for i in range(METEO_MAX):  #i = 0,1,2,...,29
        mx[i] = random.randint(0,480)   #random.randint 함수 [Start, End]인자를 주면 그 사이의 정수를 랜덤하게 생성
        my[i] = random.randint(-640,0)  #화면 왼쪽으로 랜덤하게 생성
...
def move_enemy():   #유성을 이동
    ...
    for i in range(METEO_MAX):  #i=0,1,2,...,29 (30개 유성에 대해서)
        ...
        if my[i] > 660: #유성이 화면 아래로 벗어나면 다시 생성
            mx[i] = random.randint(0, 400)  # random.randint 함수 [Start, End]인자를 주면 그 사이의 정수를 랜덤하게 생성
            my[i] = random.randint(-640, 0)  # 화면 왼쪽으로 랜덤하게 생성
...
root.bind('<KeyRelease>', key_up)    #키를 뗐을 때 실행되는 key_up 함수 지정
canvas = tkinter.Canvas(width=480, height=640)
canvas.pack()
img_player = [tkinter.PhotoImage(file='Chapter2/starship0.png'),
              tkinter.PhotoImage(file='Chapter2/starship1.png')]   #우주선 이미지 리스트
img_enemy = tkinter.PhotoImage(file='Chapter2/meteo.png')
img_bg = tkinter.PhotoImage(file='Chapter2/cosmo.png')
main()
root.mainloop()
