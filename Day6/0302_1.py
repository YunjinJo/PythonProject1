...
def draw_screen():          #미로 그리기 함수
    ...
    canvas.create_image(pen_x,pen_y,image=img_pen,tag='SCREEN') #펜펜 이미지 표시
...
def move_penpen():
    ...
    if key == 'Up':                                 #key symbol 중에서 'Up' 위쪽 화살표 눌렀을 때
        if check_wall(pen_x,pen_y,DIR_UP) == False: #위쪽 방향이 벽인지 검사
...
img_pen = tkinter.PhotoImage(file='Chapter3/image_penpen/pen03.png')    #펜펜 이미지 로딩
